...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        if (rank == 0)
            printf("Usage: mpirun -np <procs> %s <graph_file> <threads>\n",
                   argv[0]);
        MPI_Finalize();
        return 1;
    }
    ...
    if (rank == 0) {
        ...
        printf("========================================\n");
        printf("  Bellman-Ford Hybrid MPI+OpenMP\n");
        printf("========================================\n");
    ...
    if (rank == 0) {
        ...
        FILE* rf = fopen("results/hybrid_results.txt", "a");
        if (rf) {
            fprintf(rf, "V=%d E=%d procs=%d threads=%d time=%.6f sec\n",
                    V, E, size, num_threads, elapsed);
            fclose(rf);
            printf("\nTiming saved to results/hybrid_results.txt\n");
        }
        printf("========================================\n");
    }
